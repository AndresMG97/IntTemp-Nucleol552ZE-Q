...
#define TEMP_CAL1                          30
#define TS_CAL1			                      ((uint16_t*) (0x0BFA05A8))
#define TS_CAL2			                      ((uint16_t*) (0x0BFA05CA))
#define VREFINTCAL							  ((uint16_t*) (0x0BFA05AA))
/* USER CODE END PD */
...
void temp_meas(int inc)
{
	 ...
	 select_ADC_TEMP();
	 HAL_ADC_Start(&hadc1);
	 HAL_ADC_PollForConversion(&hadc1, 1000); // Poll ADC1 Perihperal & TimeOut = 1mSec
	 ...
	 tempf = (((float)TEMP_CAL2-(float)TEMP_CAL1) / (TC2 - TC1)) * (ADCstep_temp - TC1) + 30.0F;
